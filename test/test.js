...
describe('Qiscus SDK Core', () => {
  describe('Comment', () => {
    ...
    it('should return a Comment object when instantiated', () => {
      expect(comment).to.be.an.instanceOf(Comment)
    })
    it('should escape special char in message', () => {
      expect(comment.message).be.equal('&lt;script&gt;alert(\'test\')&lt;/script&gt;')
      expect(comment).to.have.property('id')
    })
    it('should return false for properties isPending, isFailed', () => {
      expect(comment.isPending).to.be.false
      expect(comment.isFailed).to.be.false
    })
    it('should return true for property isAttachment when the message have [file]...[/file] directive', () => {
      expect(imageComment.isAttachment(imageComment.message)).to.be.true
    })
    it('should return true for method isImageAttachment when it is an image', () => {
      expect(imageComment.isImageAttachment(imageComment.message)).to.be.true
    })
    it('should silently fail when the url of the file is invalid', () => {
    ...
    it('should return correct url of [file][/file] directive', () => {
      expect(imageComment.getAttachmentURI('[file]http://www.url.com/gambar.jpg[/file]')).be.equal('http://www.url.com/gambar.jpg')
    })
    it('should give default type of `text` when message type is not supported', () => {
      expect(comment.type).be.equal('text')
    })
    it('should set a unique id if `attachUniqueId` is called', () => {
