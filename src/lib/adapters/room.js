export default class RoomAdapter {
  ...
  getOrCreateRoom (email, options, distinctId) {
    ...
    return this.HTTPAdapter.post(`api/v2/sdk/get_or_create_room_with_target`, params).then(res => {
  ...
  getRoomById (id) {
    return this.HTTPAdapter.get(`api/v2/mobile/get_room_by_id?id=${id}`).then(res => res.body)
  }
  ...
  getOrCreateRoomByUniqueId (id, name, avatarURL) {
    ...
    return this.HTTPAdapter.post(`api/v2/mobile/get_or_create_room_with_unique_id`, params).then(
      res => {
        if (res.body.status !== 200) return Promise.reject(res)
        const room = res.body.results.room
        room.avatar = room.avatar_url
  ...
  createRoom (name, emails, opts = {}, optionalData = {}) {
    ...
    return this.HTTPAdapter.post(`api/v2/mobile/create_room`, body).then(res => {
      if (res.body.status !== 200) return Promise.reject(res)
      const room = res.body.results.room
  ...
  updateRoom (args) {
    ...
    return this.HTTPAdapter.post(`api/v2/mobile/update_room`, params).then(res => {
      if (res.body.status !== 200) return Promise.reject(res)
  ...
  addParticipantsToGroup (roomId, emails = []) {
    ...
    return this.HTTPAdapter.post(`api/v2/mobile/add_room_participants`, params).then(res => {
      if (res.body.status !== 200) return Promise.reject(res)
  ...
  removeParticipantsFromGroup (roomId, emails = []) {
    ...
    return this.HTTPAdapter.post(`api/v2/mobile/remove_room_participants`, params).then(res => {
      if (res.body.status !== 200) return Promise.reject(res)
