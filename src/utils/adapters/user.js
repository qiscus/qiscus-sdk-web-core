  ...
  */
  ...
  updateCommentStatus(roomId, lastReadCommentId, lastReceivedCommentId) {
    ...
    if (lastReadCommentId) {
      body.last_comment_read_id = lastReadCommentId;
    }
  ...
  loadRoomList(params = {}) {
    ...
    if (params.show_participants) {
      body += `&show_participants=${params.show_participants || true}`;
    }
    ...
    if (params.show_empty) {
      body += `&show_empty=${params.show_empty}`;
    }
  ...
  getRoomsInfo(opts) {
    ...
    if (opts.room_ids) {
      body.room_id = opts.room_ids;
    }

    if (opts.room_unique_ids) {
      body.room_unique_id = opts.room_unique_ids;
    }

    if (opts.show_participants) {
      body.show_participants = opts.show_participants;
    }

    if (opts.show_removed) {
      body.show_removed = opts.show_removed;
    }
  ...
  loadComments(topicId, options) {
    ...
    if (options.last_comment_id) {
      params += `&last_comment_id=${options.last_comment_id}`;
    }
