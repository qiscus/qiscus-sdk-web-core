...
class QiscusSDK {
  ...
  init(config) {
    ...
    this.syncAdapter.events.on('message.delivered', (message) => {
      if (this.selected != null) {
        const _message = this.selected.comments.find(it => it.unique_id === message.comment_unique_id)
        if (_message != null) {
          _message.markAsDelivered()
          this.events.emit('comment-delivered', {
            actor: _message.username_real,
            comment: _message
          })
        }
      }
    ...
    this.syncAdapter.events.on('message.deleted', (data) => {
      data.deleted_messages.forEach(it => this.events.emit('comment-deleted', {
        roomId: it.room_id,
    ...
    this.syncAdapter.events.on('room.deleted', (data) => {
      data.deleted_rooms.forEach(room => this.events.emit('room-cleared', room))
    })
  ...
  setEventListeners() {
    ...
    self.events.on('login-success', (response) => {
      ...
      setInterval(() => this.realtimeAdapter.publishPresence(this.user_id), 5000)
  ...
  activateSync() {
    ...
    self.isSynced = true
    self.httpsync = setInterval(() => self.synchronize(), 5000)
    self.eventsync = setInterval(() => self.synchronizeEvent(), 5000)
  }
