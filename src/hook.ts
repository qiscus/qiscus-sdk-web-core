...
export function hookAdapterFactory() {
  ...
  function intercept<T extends unknown>(hook: string, callback: (data: T) => T): () => void {
  ...
  function triggerBeforeReceived$<T extends unknown>(payload: T): Stream<T> {
    return xs.fromPromise(trigger(Hooks.MESSAGE_BEFORE_RECEIVED, payload))
  }
