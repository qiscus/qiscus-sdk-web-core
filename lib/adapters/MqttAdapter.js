      ...
      if(topic.length == 2) {
        ...
        if(topic[1] == 'c') {
          ...
          parsedMessage.payload.data.deleted_messages.map(msgRoom => {
            self.context.emit('comment-deleted', {roomId: msgRoom.room_id, commentUniqueIds: msgRoom.message_unique_ids, isForEveryone: true, isHard: parsedMessage.payload.data.is_hard_delete});
          })
        }
