    ...
    this.mqtt.on('message', function(topic, message) {
      ...
      if(topic.length == 2) {
        ...
        if(topic[1] == 'c') {
          ...
          parsedMessage.data.deletedMessages.map(msgRoom => {
            msgRoom.message_unique_ids.map(unique_id => {
              self.context.emit('comment-deleted', {roomId: msgRoom.room_id, commentId: unique_id})
            })
          })
        }
