  ...
  constructor(url, callbacks, context) {
    ...
    this.mqtt.on('message', function(topic, message) {
      ...
      if(topic.length == 2) {
        ...
        // find the comment that need to be altered
        // cari comment ini di seluruh room
        const rooms = self.context.rooms;
        let commentToFind = null;
        let commentRoom = null;
        const messageData =  message.split(":");
        Loop1:
        for(let i=0; i<rooms.length; i++) {
          Loop2:
          for(let j=0; j<rooms[i].comments.length; j++) {
            let commentData = rooms[i].comments[j];
        ...
        // const commentToFind = self.context.selected.find(selectedComment => {
        // // const commentToFind = self.context.selected.comments.find(selectedComment => {
        //   return (
        //     message.split(":")[1]
        //       ? selectedComment.unique_id == message.split(":")[1]
        ...
        //   )
        // })
        if (commentToFind && commentToFind.status!='read' && commentRoom){
          if(topic[3] == commentToFind.username_real) return false;
          const options = {
            participants: self.context.selected.participants,
            actor: topic[3],
